...
function showData(data) {
  ...
  xmlDoc.querySelectorAll('item').forEach((e,i) => {
    txt += `<details><summary onclick="if(!this.parentElement.querySelector('p')){const para=document.createElement('p');this.parentElement.appendChild(para);const el=document.createElement('a');el.setAttribute('href','${e.querySelector('link').innerHTML}');el.setAttribute('target','_blank');el.setAttribute('rel','noreferrer');el.innerHTML='${e.querySelector('title').innerHTML.replace(/\"/g, "&quot;")}: ${e.querySelector('description').innerHTML.replace(/\"/g, "&quot;")}';para.appendChild(el)}"><span class="time">${showTime(i,getTime(e.querySelector('pubDate').innerHTML))}</span> ${e.querySelector('title').innerHTML}</summary></details>`
  })
